...
public class laserArrayManager : MonoBehaviour
{
    ...
    {
        if (pillar<0 || pillar >= pillars.Length) return null;

        laserArray[] pillarArray = pillars[pillar];

        if (pillarArray == null) return null;
        if (slot < 0 || slot >= pillarArray.Length) return null;
    ...
    private laserArray[] getPillar(pillarColor color)
    {
        ...
        if (colIndex< 0 || colIndex >= pillars.Length) return null;
    ...
    private int getPillarIndex(pillarColor color)
    {
        switch (color)
        {
            case pillarColor.Red: return 0;
            case pillarColor.Blue: return 1;
            case pillarColor.Green: return 2;
            case pillarColor.Yellow: return 3;
        }
    ...
    public bool getIsFiring(pillarColor color)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (target.getIsDeployed) return true;
        }
    ...
    public void startPattern(int patternIndex)
    {
        ...
        if (chosen == null || chosen.steps == null || chosen.steps.Length == 0) return;
